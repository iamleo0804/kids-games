...
import { LuArrowLeft, LuVolume2, LuCheck, LuX, LuPartyPopper } from 'react-icons/lu';
import './QuizPlay.css';
...
const QuizPlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    while (generated.length < 10) {
      if (subjectId === 'math') {
        let num1: number, num2: number, ans: number, operator: string, audioOp: string;
        ...
        const options = [ans, ...Array.from(wrongOptions)].sort(() => Math.random() - 0.5).map(String);
        generated.push({
  ...
  if (isCompleted) {
    return (
      <div className="quiz-result-page">
        <LuPartyPopper className="result-icon animate-bounce" size={100} color="var(--color-secondary)" />
    ...
    <div className="quiz-play-page">
      <header className="quiz-header">
        ...
        <div className="progress-container">
          ...
          <span className="progress-text">第 {currentIndex + 1} / {questions.length} 題</span>
        </div>
      ...
      <main className="quiz-main">
        <div className="question-card">
          ...
          <button className="speak-btn" onClick={() => handleSpeak(currentQuestion.audioText)}>
            <LuVolume2 size={32} />
          </button>
        </div>
